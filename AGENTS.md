...
- Tests Rust : `cargo test --manifest-path src-tauri/Cargo.toml`
  ...
  avec `node scripts/make-fixtures.mjs`.
...
> `ecc` scopé OFF sur Sift depuis 2026-07-01 (`.Codex/settings.local.json`, coût
...
`Codex-in-chrome` reste acceptable pour un point précis, jamais une session
...
fenêtre WebView2 — Codex peut alors inspecter/screenshot le code `inTauri`
réel par WebSocket (Node natif, zéro dépendance), sans `computer-use` ni
`Codex-in-chrome`. Reste ponctuel (2-3 appels ciblés), jamais une session
